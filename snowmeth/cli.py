...
from .exceptions import SnowmethError
import json
...
def _handle_step_9_scene_expansions(story, workflow, renderer):
    ...
    for i, (scene_key, expansion_data) in enumerate(scene_expansions.items(), 1):
        ...
        if isinstance(expansion_data, dict):
            ...
            if setting:
                click.echo(
                    f"  Setting: {setting[:100]}{'...' if len(setting) > 100 else ''}"
                )
            ...
            click.echo(
                f"  {str(expansion_data)[:200]}{'...' if len(str(expansion_data)) > 200 else ''}"
            )
...
def analyze():
    ...
    try:
        ...
        try:
            ...
            if isinstance(analysis_data, str):
                try:
                    ...
                    click.echo(
                        f"DEBUG: Second parse result type: {type(analysis_data)}"
                    )
        ...
        if not isinstance(analysis_data, dict):
            click.echo(
                f"❌ Expected analysis to be a JSON object, got: {type(analysis_data)}"
            )
            click.echo(f"Raw analysis saved to: {analysis_file}")
            click.echo("Raw response:")
            click.echo(analysis)
        ...
        if total_issues > 0 or scene_count > 0:
            ...
            if scene_count > 0:
                scene_numbers = [
                    str(s.get("scene_number", "?")) for s in scene_improvements
                ]
                click.echo(
                    f"  • Run 'snowmeth improve-all' to fix scenes: {', '.join(scene_numbers)}"
                )
            else:
                click.echo(
                    "  • Run 'snowmeth improve-all' to fix all issues automatically"
                )
...
def _get_change_summary(old_scene, new_scene):
    ...
    if "Placeholder" in old_title and "Placeholder" not in new_title:
        ...
        if "Placeholder" in old_title or (
            "Scene " in old_title and old_title.count(" ") <= 2
        ):
...
def improve_all(auto):
    ...
    try:
        ...
        scenes_to_improve = analysis_workflow.identify_scenes_needing_improvement(
            story, analysis_data
        )
        ...
        if not scenes_to_improve:
            ...
            if not scene_input:
                ...
                if step9_content:
                    ...
                    scenes_to_improve = [
                        int(key.split("_")[1])
                        for key in current_expansions.keys()
                ...
                try:
                    ...
                    click.echo(
                        "Error: Please enter scene numbers as integers (e.g., '1,3,5')"
                    )
                    return
        ...
        click.echo(
            f"📝 Will improve {len(scenes_to_improve)} scenes: {', '.join(map(str, scenes_to_improve))}"
        )
        ...
        improved_count, errors = analysis_workflow.improve_scenes(
            story, scenes_to_improve, analysis_data
        )
...
def improve(scene_numbers):
    ...
    try:
        ...
        if not step9_content:
            click.echo(
                "No Step 9 content found. Run 'snowmeth next' to generate scene expansions first."
            )
            return
        ...
        for scene_num in scene_nums:
            try:
                ...
                improved_scene = workflow.improve_scene(
                    story, scene_num, improvement_guidance
                )
                ...
                try:
                    ...
                    if "Placeholder" in old_title and "Placeholder" not in new_title:
                        ...
                        old_beats = len(
                            current_expansions[scene_key].get("key_beats", [])
                        )
