...
async def migrate_database():
    ...
    try:
        async with engine.begin() as conn:
            ...
            await conn.execute(text("ALTER TABLE stories ADD COLUMN chapters JSON DEFAULT '{}';"))
            
            # Update all existing rows to have empty chapters object
            await conn.execute(text("UPDATE stories SET chapters = '{}' WHERE chapters IS NULL;"))
            
            print("✅ Migration completed successfully!")
            
            # Verify the migration worked
            result = await conn.execute(text("SELECT COUNT(*) FROM stories WHERE chapters IS NOT NULL;"))
